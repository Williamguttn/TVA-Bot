...
async function fetchOrThrow(endpoint, params) {
    ensureConfigured();
...
async function getPlayerThumbnail(userId, size = "100x100", format = "png", isCircular = false, _type = "Headshot") {
...
async function setRank(groupId, userId, rank) {
    ...
    const role = roles.find((entry) => entry.rank === rank);
    if (!role) {
...
async function fetchIdFromUsername(roblox, interaction, username, client) {
    let id;
    try {
...
async function fetchPlayerInfo(roblox, interaction, id) {
    let playerInfo;
    try {
...
async function fetchThumbnail(roblox, interaction, id) {
    let thumbnail;
    try {
...
async function fetchGroupRoles(roblox, interaction, id) {
    let roles;
    try {
...
async function fetchPlayerGroups(roblox, interaction, id) {
    let groups;
    try {
...
async function fetchUserGroupRank(roblox, interaction, groupId, userId) {
    let rank;
    try {
...
async function setGroupRank(roblox, interaction, groupId, userId, rank) {
    try {
...
async function fetchUsernameFromId(roblox, interaction, id) {
    let username;
    try {
