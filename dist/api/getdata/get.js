...
module.exports = async function (req, res, db) {
    ...
    let data = await doSql(db, "SELECT score FROM users WHERE roblox_id = ?", [userId]);
    if (data.length === 0) {
    ...
    data[0].medals = [];
    try {
        const medalNames = await doSql(db, `SELECT name FROM medals 
            JOIN user_medals ON medals.display_order = user_medals.medal_id
            WHERE user_medals.roblox_id = ?`, [userId]);
